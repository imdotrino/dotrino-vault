...
- ~~**`exp` corto por defecto**~~ **— YA NO APLICA (2026-08-31).** El cert lleva `seq` y muere con el acta, así que no hay `exp` que acortar ni renovación que exigir: quitarle el permiso al ladrón surte efecto al subir el acta, sin esperar a nada. Lo que sigue en pie es que quien nunca vio el acta nueva puede aceptar la vieja (R1). Decía: bajar a horas para `vault:store`/`vault:read` y exigir **renovación autenticada periódica** (heartbeat firmado por `D`, que el daemon solo concede si el `deviceId` no está revocado). Un cert robado caduca pronto **sin** depender de que el daemon esté online en el instante del robo.
