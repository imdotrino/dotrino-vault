...
function help () {
  ...
  ssh-agent [--ns <ns>] [--socket <ruta>]
                         agente SSH delgado: las llaves viven en el teléfono; cada firma es
                         un pedido a la bóveda. Pon SSH_AUTH_SOCK en la ruta que imprime.
...
async function cmdRun () {
  ...
  const { injected, overridden } = await loadEnv({ ns: flag('ns'), onPending })
...
/**
 * Agente SSH DELGADO. Aquí no hay ninguna llave: el `ssh` del usuario pide firmar, esto
 * convierte el reto en un pedido a la bóveda (la que enroló este servicio, esté donde
 * esté) y el teléfono firma. Las públicas se cachean 30 s para no preguntar en cada `ssh`.
 */
async function cmdSshAgent () {
  const { startSshAgent, defaultSocketPath } = await import('../src/sshAgent.js')
  const { listSshKeys, requestSshSign } = await import('../src/service.js')
  const ns = flag('ns') || undefined
  const { serviceDir, resolveNs } = await import('../src/env.js')
  const dir = flag('dir') || serviceDir(resolveNs(ns))
  const socketPath = flag('socket') || defaultSocketPath(dir)
  let cache = { at: 0, keys: [] }
  const keys = async () => {
    if (Date.now() - cache.at > 30 * 1000) { try { cache = { at: Date.now(), keys: await listSshKeys({ dir }) } } catch (e) { console.error('[dotrino-env] could not list keys: ' + e.message) } }
    return cache.keys
  }
  const vault = {
    sshKeys: () => cache.keys,
    requestSshSign: ({ keyId, data }) => requestSshSign({ dir }, { keyId, data, onPending: () => console.error('[dotrino-env] ssh: waiting for approval on your phone…') })
  }
  await keys()
  const agent = startSshAgent({ socketPath, vault: () => vault, log: console.error, refresh: keys })
  console.error('export SSH_AUTH_SOCK=%s', agent.socketPath)
