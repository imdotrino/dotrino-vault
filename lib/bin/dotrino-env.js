...
/**
 * La invitación que imprime `dotrino-vault pair` viene en varias formas: JSON
 * crudo, base64url del payload, URL de profile con `#vault=<json>` (nuevo, menor)
 * o `#vault=<b64>` (viejo). Aceptamos todas para que el operador pegue lo que tenga.
 */
function parseInvite (raw) {
  const s = String(raw || '').trim()
  if (!s) throw new Error('invitación vacía')
  if (s.startsWith('{')) return JSON.parse(s)
  const payload = s.includes('#vault=') ? s.split('#vault=')[1] : (s.includes('#') ? s.split('#').pop() : s)
  // Nuevo: JSON crudo en el fragmento (menor). Viejo: base64url.
  try { const o = JSON.parse(payload); if (o && typeof o === 'object') return o } catch {}
