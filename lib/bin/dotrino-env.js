 ...
 *
 *   dotrino-env enroll --profile <p> --ns <ns> [--qr <inv>]   enrola ESTA máquina (una vez)
 ...
 *   2. aquí:          dotrino-env enroll --profile <p> --ns <ns>  (pegas la invitación; se MUESTRA un código)
...
import { loadEnv, serviceDir, serviceRoot, listEnrolled, resolveNs, resolveEnvProfile, isValidEnvProfile } from '../src/env.js'
...
function help () {
  ...
  enroll --profile <perfil> --ns <ns> [--code <invitación>] [--dir <dir>]
  ...
  run --profile <perfil> [--ns <ns>] -- <cmd> [args…]
...
Entorno: DOTRINO_NS · DOTRINO_ENV_PROFILE · DOTRINO_ENV_DIR · DOTRINO_ENV_HOME · DOTRINO_ENV_QUIET
...
/**
 * El PERFIL LOCAL, obligatorio. Es una etiqueta de ESTA máquina para agrupar lo que
 * pertenece a una bóveda: `~/.dotrino/service/<perfil>/<ns>`. No viaja, no entra en el
 * acta y no se compara con el perfil de la bóveda — solo evita que dos bóvedas con un
 * cajón del mismo nombre se pisen el directorio, que antes dejaba al primero inservible.
 */
function perfilLocal () {
  const p = resolveEnvProfile(flag('profile'))
  if (!p) {
    console.error('falta --profile <nombre>  (o DOTRINO_ENV_PROFILE)')
    console.error('')
    console.error('  Es una ETIQUETA TUYA para agrupar lo de una bóveda, no el perfil del vault:')
    console.error('    ~/.dotrino/service/<perfil>/<ns>')
    console.error('  Sirve para que dos bóvedas puedan tener un cajón llamado igual.')
    process.exit(2)
  }
  if (!isValidEnvProfile(p)) { console.error('perfil inválido: usa [a-z0-9-]{1,32}'); process.exit(2) }
  return p
}
...
async function cmdEnroll () {
  ...
  const dir = flag('dir') || serviceDir(perfil, ns)
...
function cmdStatus () {
  ...
  if (!found.length) {
    console.log('Ningún servicio enrolado en %s\n  Enrola uno:  dotrino-env enroll --profile <perfil> --ns <ns>', serviceRoot())
    return
  }
  for (const e of found) {
    ...
    if (e.legacy) {
      ...
      console.log('%s  ⚠ del formato anterior, sin perfil — no se usa', e.ns)
      console.log('  muévelo:  mkdir -p %s/<perfil> && mv %s %s/<perfil>/', serviceRoot(), viejo, serviceRoot())
      continue
    }
    const dir = serviceDir(e.profile, e.ns)
    ...
    console.log('%s/%s\n  dir:   %s\n  vault: %s…\n  scope: %s\n  cert:  %s',
      e.profile, e.ns, dir, String(id.iss).slice(0, 24), (c?.scope || []).join(', '), papel)
  }
}
...
async function cmdCheck () {
  ...
  const secrets = await fetchSecrets({ dir: serviceDir(perfilLocal(), ns), ns })
...
async function cmdRun () {
  ...
  if (!cmd.length) { console.error('uso: dotrino-env run --profile <perfil> [--ns <ns>] -- <cmd> [args…]'); process.exit(2) }
  const onPending = () => console.error('[dotrino-env] waiting for approval on your phone…')
  const { injected, overridden } = await loadEnv({ ns: flag('ns'), profile: perfilLocal(), onPending })
...
async function cmdSshAgent () {
  ...
  const dir = flag('dir') || serviceDir(perfilLocal(), ns)
