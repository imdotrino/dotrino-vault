...
function help () {
  ...
  run --vault <bóveda> [--ns <ns>] -- <cmd> [args…]
...
async function cmdEnroll () {
  ...
  const perfil = etiquetaBoveda()
  const dir = flag('dir') || serviceDir(bov, ns)
...
async function cmdRun () {
  ...
  if (!cmd.length) { console.error('uso: dotrino-env run --vault <bóveda> [--ns <ns>] -- <cmd> [args…]'); process.exit(2) }
  ...
  const { injected, overridden } = await loadEnv({ ns: flag('ns'), vault: etiquetaBoveda(), onPending, onRetry })
