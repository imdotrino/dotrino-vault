...
import { selfContext, commandLine } from '../src/proc.js'
import { updateNotice } from '@dotrino/update/notice'
import { PKG_VERSION as VERSION } from '../src/version.js'

const argv = process.argv.slice(2)

/**
 * ¿HAY UN dotrino-env MÁS NUEVO? (CONVENCIONES §15)
 *
 * La regla que manda en un comando: **avisar no puede hacer lenta la orden**. Por eso la
 * consulta ARRANCA aquí, antes de hacer nada, y se recoge al terminar: mientras corre lo que
 * el usuario pidió —un `run` dura lo que dure su programa— la pregunta ya está contestada, y
 * lo normal es que no cueste ni un milisegundo. Una vez al día; el resto sale de caché.
 *
 * Se compara contra `@dotrino/env`, que es lo que la gente instala, con la versión del
 * código que corre: son la misma por construcción (`packaging/release-env.mjs` las fija
 * iguales al publicar).
 *
 * La línea va por STDERR: `dotrino-env run` le cede stdout a otro programa, y colarle ahí
 * una cortesía es romperle la tubería a quien lo esté canalizando.
 */
const avisoVersion = updateNotice({
  current: VERSION, source: 'npm', pkg: '@dotrino/env', product: 'dotrino-env'
}).catch(() => null)

/** Lo dice, si hay algo que decir, con un tope: nunca retiene la salida más de 1,5 s. */
let avisoDicho = false
async function decirAvisoVersion () {
  if (avisoDicho) return
  avisoDicho = true
  const linea = await Promise.race([avisoVersion, new Promise((resolve) => setTimeout(() => resolve(null), 1500).unref())])
  if (linea) process.stderr.write(`\n[dotrino-env] ${linea} · actualiza con: npm i -g @dotrino/env\n`)
}
const flag = (name) => { const i = argv.indexOf('--' + name); return i >= 0 ? argv[i + 1] : undefined }

/**
 * QUÉ VA A VER EN EL TELÉFONO, dicho aquí también.
 *
 * El pedido que llega al aparato que aprueba enseña el comando y la carpeta; si por aquí solo
 * se imprime «waiting for approval», no hay forma de emparejar lo que uno acaba de lanzar con
 * lo que suena en el bolsillo — y con dos ventanas abiertas eso es exactamente lo que hay que
 * distinguir antes de decir que sí.
 */
function esperandoAprobacion () {
  const ctx = selfContext()
  console.error('[dotrino-env] the vault asks for approval — approve it on a device with «aprueba» (up to 5 min)')
  console.error('[dotrino-env]   %s', commandLine(ctx, 160))
  console.error('[dotrino-env]   from %s', ctx.cwd || '?')
}

function help () {
  console.log(`dotrino-env — credenciales del vault en vez del .env

  enroll --ns <ns> [--vault <bóveda>] [--code <invitación>] [--dir <dir>]
        (sin --vault se usa el NOMBRE DE LA CUENTA, que la bóveda dice al enrolar; solo
         hace falta darla si quieres otra etiqueta, o si el mismo <ns> está en dos bóvedas)
  ...
  check [--ns <ns>] [--public]
                         pide los secretos al vault y lista sus NOMBRES (nunca los valores)
                         con --public no pide aprobación: solo mira las públicas
...
function etiquetaBoveda ({ exigida = false } = {}) {
  ...
  // EN MINÚSCULAS, SIN PROTESTAR. La etiqueta es el nombre de una carpeta de esta máquina
  // y nada más, así que las mayúsculas no significan nada aquí — pero el perfil sí se llama
  // «Cepi», con mayúscula, y es lo que uno teclea. Rechazarlo era fricción sin nada a
  // cambio, y el mensaje solo repetía la regla en vez de decir qué escribir.
  const norm = v.toLowerCase()
  if (!isValidVaultLabel(norm)) {
    console.error('etiqueta inválida: «%s»', v)
    console.error('  es el nombre de una carpeta de esta máquina: letras, números y guiones (máx. 32).')
    console.error('  ej.:  --vault cepi')
    process.exit(2)
  }
  return norm
}
...
async function cmdEnroll () {
  ...
  let qr = parseInvite((flag('qr') || flag('code')) || await readInvite())
  // SIN `--vault`, SE LO PREGUNTA A LA BÓVEDA (dueño, 2026-09-03: «en el enroll si no se
  // pone vault debería usar el nombre del perfil»). Al enrolar ya sabes contra qué bóveda
  // vas —la invitación lo dice, o ella lo contesta—, así que pedir la etiqueta era hacer
  // teclear un dato que el programa tiene delante.
  //
  // La etiqueta es solo el nombre de una carpeta de esta máquina, así que el nombre de la
  // cuenta se lleva a minúsculas y se le quitan los caracteres que no valen. Si de ahí no
  // sale nada utilizable, se pide — no se inventa un nombre.
  let boveda = etiquetaBoveda()
  if (!boveda) {
    const { vaultHello } = await import('../src/service.js')
    // Se REUSA el qr que vuelve del hello: la cita se quema al canjearla, así que
    // preguntar el nombre y después enrolar con el qr original gastaba la invitación
    // en la pregunta y dejaba al enrolamiento sin nada que canjear.
    qr = await vaultHello(qr).catch(() => qr)
    const nombre = qr.acct || ''
    boveda = String(nombre || '').toLowerCase().replace(/[^a-z0-9-]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 32)
    if (!boveda) {
      console.error('esa bóveda no dice cómo se llama su cuenta: dime tú la etiqueta')
      console.error('  ej.:  dotrino-env enroll --ns %s --vault mi-boveda --code …', ns)
      process.exit(2)
    }
    console.log('  bóveda: «%s» (el nombre de la cuenta; se guarda en ~/.dotrino/service/%s/%s)', boveda, boveda, ns)
  }
...
function cmdStatus () {
  ...
  if (!found.length) {
    console.log('Ningún servicio enrolado en %s\n  Enrola uno:  dotrino-env enroll --ns <ns> --code "<invitación>"', serviceRoot())
    return
  }
...
async function cmdCheck () {
  ...
  // `--public` también aquí: ver QUÉ hay no debería costarle una aprobación al dueño. Sin
  // él se piden todas, que es lo que hace falta para comprobar de verdad un arranque.
  const publicOnly = argv.includes('--public')
  // Y SE DICE QUE ESTÁ ESPERANDO. Sin este aviso `check` se queda mudo cinco minutos —el
  // plazo de la aprobación— y desde fuera eso es un cuelgue: ni una línea que diga que la
  // bóveda contestó, que lo que falta es un permiso, ni cuánto va a esperar. `run` sí lo
  // decía; `check`, que es el comando de DIAGNÓSTICO, era el único que no. Le pasó al dueño
  // el 2026-09-05 con un cajón cuyo servicio no tenía a nadie que pudiera aprobarle.
  const onPending = () => esperandoAprobacion()
  const secrets = await fetchSecrets({ dir: serviceDir(etiquetaBoveda(), ns), ns, publicOnly, onPending })
  const keys = Object.keys(secrets)
  console.log('ns "%s": %d %s%s', ns, keys.length,
    publicOnly ? 'variable(s) pública(s)' : 'secreto(s)', keys.length ? ':' : '')
...
async function cmdRun () {
  ...
  // El plazo se imprime en minutos cuando lo es: un «retrying in 310s» parece un cuelgue,
  // y no lo es — es el pedido que ya timbró, esperando a vencer antes de volver a sonar.
  const humanDelay = (ms) => (ms >= 90000 ? `${Math.round(ms / 60000)} min` : `${Math.round(ms / 1000)}s`)
  const onRetry = (e, ms) => console.error('[dotrino-env] failed: %s — retrying in %s', e.message, humanDelay(ms))
  ...
  // Al terminar el programa, no antes: mientras corría, la consulta ya se contestó.
  child.on('exit', async (code, signal) => { await decirAvisoVersion(); process.exit(signal ? 1 : (code ?? 0)) })
}
...
async function cmdSshAgent () {
  ...
  const secrets = await waitForSecrets({ dir, ns, onPending: () => esperandoAprobacion() })
  const keys = []
...
run()
  // `run` y `ssh-agent` NO avisan aquí, y hay que hacerlo cumplir, no solo decirlo: `cmdRun`
  // termina en cuanto LANZA el programa, así que este `.then` lo diría mientras el programa
  // aún corre, mezclado con su salida. `run` avisa cuando su hijo sale; `ssh-agent` no
  // termina nunca. Aquí llegan las órdenes cortas.
  .then(() => (['run', 'ssh-agent'].includes(argv[0]) ? null : decirAvisoVersion()))
