 ...
 *   bits 0-2  versión · bit 3  modo (join/adopt)
 *
 * Sin llave, sin proxy y sin nombre de cuenta: todo eso llega en la respuesta de la
 * bóveda, así que un nombre largo ya no agranda el QR. 13 bytes → 18 caracteres.
 */
...
function shortEncode (qr) {
  ...
  const known = new Set(['v', 'conn', 'sn', 'm'])
  if (Object.keys(qr).some((k) => !known.has(k))) return null
  const out = [qr.v | (mode << 3), ...[...conn].map((c) => c.charCodeAt(0)), ...hexToBytes(qr.sn)]
  return bytesToB64url(Uint8Array.from(out))
}
...
function shortDecode (text) {
  ...
  const head = b[0]
  if (head & 0xf0) return null
  const conn = String.fromCharCode(...b.subarray(1, 1 + CONN_TOKEN_LEN))
  return { v: head & 7, conn, sn: bytesToHex(b.subarray(1 + CONN_TOKEN_LEN)), m: MODES[(head >> 3) & 1] }
}
