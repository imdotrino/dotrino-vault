...
export async function connectLocal ({ masterId, timeoutMs = 3000 } = {}) {
  ...
  return {
    ...
    //
    // LAS DOS FORMAS, porque quien llama no sabe por dónde salió. `freshClient` devuelve
    // esto o un `WebSocketProxyClient` según haya socket o no, y el de arriba llama al
    // método sin más: si aquí falta el que el pilar usa ahora, el escalón 1 del transporte
    // —el más rápido, y el que corre en la máquina del dueño— muere con «client.identifyAs
    // is not a function» mientras por el proxio todo va bien. Que sea un no-op no lo hace
    // opcional: es la forma del cliente, y tiene que estar entera.
    identify: async () => ({ ok: true }),
    identifyAs: async () => ({ ok: true }),
    on: (ev, fn) => {
