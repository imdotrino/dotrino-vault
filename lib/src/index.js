...
export async function startDeviceVault (identity, { proxyUrl, client: injectedClient } = {}) {
   ...
   */
  async function autorizar (from, p, expectedScope) {
    const d = p?.data
  ...
  /**
   * FIRMAR en nombre de la identidad. Es la razón de ser de una bóveda, y faltaba: un
  ...
  async function handleSign (from, p) {
    const chk = await autorizar(from, p, SCOPE.SIGN)
    if (!chk) return
  ...
  async function handleGet (from, p) {
    const chk = await autorizar(from, p, SCOPE.READ)
    if (!chk) return
  ...
  async function handleStore (from, p) {
    ...
    const chk = await autorizar(from, p, SCOPE.STORE)
    if (!chk) return
    try {
   ...
   */
  const atender = (nombre, promesa, from) => Promise.resolve(promesa).catch((e) => {
    send(from, { type: MSG.ERROR, error: `${nombre}: ${e?.message || e}` })
  })

  client.on('message', (_from, p) => {
    if (!p || typeof p !== 'object') return
    // El QR corto no lleva la llave: el aparato la pide con un HELLO presentando el `sn`.
    if (p.type === MSG.HELLO) atender('hello', desk.handleHello(_from, p), _from)
    else if (p.type === MSG.ENROLL) atender('enroll', desk.handleEnroll(_from, p), _from)
    // Camino A: el aparato devuelve su acta sellada admitiendo a esta bóveda.
    else if (p.type === MSG.ACTA_SEALED) atender('acta', desk.handleActaSealed(_from, p), _from)
    else if (p.type === MSG.RENEW) atender('renew', handleRenew(_from, p), _from)
    else if (p.type === MSG.DEVICES) atender('devices', handleDevices(_from, p), _from)
    ...
    else if (p.type === MSG.SIGN) atender('sign', handleSign(_from, p), _from)
    else if (p.type === MSG.GET) atender('get', handleGet(_from, p), _from)
    else if (p.type === MSG.STORE) atender('store', handleStore(_from, p), _from)
    else if (p.type === MSG.CHECK) atender('check', handleCheck(_from, p), _from)
  })
