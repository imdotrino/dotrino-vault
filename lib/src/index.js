...
import { memberCanSign } from '@dotrino/identity/acta'
...
export async function startDeviceVault (identity, { proxyUrl, client: injectedClient } = {}) {
  ...
  async function handleSign (from, p) {
    ...
    if (!chk) return
    // MANDA EL ACTA, NO EL PAPEL. `authorise` mira el certificado, y quitarle `firma` a un
    // aparato NO reemite ni revoca su cert: seguiría diciendo `vault:sign` hasta 30 días.
    // Sin esto, retirarle el permiso lo cambiaba de firmar él a que firmara la bóveda por
    // él — o sea, no se lo retiraba. Mismo arreglo que en el daemon (`src/vault.js`).
    const record = (await identity.profileActa?.().catch(() => null))?.acta || null
    if (record && !memberCanSign(record, chk.device)) {
      return send(from, { type: MSG.ERROR, error: 'unauthorized: acta — this member does not sign' })
    }
