 ...
 * Queda fuera a propósito `activity.log`: es la bitácora de auditoría y no guarda
 * payloads (solo op, dispositivo y hora), y se quiere legible para diagnosticar.
 *
...
import { resolveKey, mintKey, readConfig, writeConfig, probe, clearCache, KekError, CONFIG_FILE, WRAPPED_FILE } from './kek.js'
...
export function kekFor (dir, { password = '' } = {}) {
  return resolveKey(dir, { password, machineKeyFn: machineKey, magic: MAGIC })
}
...
export function encryptedFilesIn (dir) {
  ...
  return names.filter((n) => {
    if (n === CONFIG_FILE || n === WRAPPED_FILE || n === SALT_FILE) return false
...
export { readConfig, writeConfig, probe, KekError, CONFIG_FILE, WRAPPED_FILE }
