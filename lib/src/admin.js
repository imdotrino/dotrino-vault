...
export function createAdminDesk ({
  ...
  async function handle (data, { signature, cert } = {}) {
    if (!data || !ops.has(data.op)) return { ok: false, error: 'admin: operación inválida' }
    if (typeof data.nonce !== 'string' || data.nonce.length < 16) {
      return { ok: false, error: 'admin: falta el nonce de un solo uso' }
    }

    const chk = await verify({ data, signature, cert })
    if (!chk?.ok) {
      audit('rejected', { what: 'admin', op: data.op, reason: chk?.reason })
      return { ok: false, error: 'no autorizado: ' + (chk?.reason || 'cert') }
    }
    ...
    if (nonceAlreadyUsed(data.nonce)) {
      ...
      return { ok: false, error: 'admin: ese nonce ya se usó' }
    }
    ...
    try {
      ...
      if (data.op === 'pair') {
        ...
        if (forbidden) {
          ...
          return { ok: false, error: 'admin: desde aquí no se concede «administra» ni claves de servicio; eso se hace en el PC' }
        }
    ...
    return { ok: false, error: 'admin: operación inválida' }
  }
