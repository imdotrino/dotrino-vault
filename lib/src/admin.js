...
/**
 * LO ÚNICO QUE SE ATIENDE CON EL CANDADO ECHADO: mirar. Ni una operación que cambie algo.
 *
 ...
 * firma un certificado de 30 días CON LA MAESTRA. O sea que el candado frenaba
 * `dotrino-vault pair` en la máquina (`daemon.js`) y no frenaba lo mismo por la red,
 * que es el camino que importa.
 *
 ...
 */
export const ADMIN_OPS_WHILE_LOCKED = Object.freeze(['pending', 'audit', 'vars'])
...
export function createAdminDesk ({
  ...
  async function handle (data, { signature, cert } = {}) {
    ...
    // CERRADA NO SE ADMINISTRA. Va después de autorizar (el candado es estado del perfil,
    ...
    try {
      // `locked` viaja aquí porque `pending` es lo que la consola ya sondea: sin esto la
      // única forma de enterarse del candado era pulsar un botón y ver el error.
      if (data.op === 'pending') return { ok: true, result: { pending: desk.listPending(), locked: !!isLocked() } }
