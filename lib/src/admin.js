 ...
 */
export const ADMIN_OPS_WHILE_LOCKED = Object.freeze(['status', 'audit', 'vars'])
...
export function createAdminDesk ({
  ...
  async function handle (data, { signature, cert } = {}) {
    ...
    try {
      // Lo mínimo que la consola necesita saber de la bóveda para explicarse: si está
      // cerrada, lo que queda (revocar, configurar) no se puede, y hay que decir por qué en
      // vez de dejar pulsar un botón para recibir un error.
      if (data.op === 'status') return { ok: true, result: { locked: !!isLocked() } }
