...
export function createEnrollDesk ({
  ...
  connToken = null,
  /**
   * Interruptor del QR CORTO (token de conexión + `sn`, sin la llave). Apagado por
   * defecto **a propósito**: el QR es lo único del protocolo que no puede negociar
   * versión —no caben las dos formas en un mismo código—, así que no se emite hasta
   * que el lado del dispositivo sepa pedir la llave (`vault.hello`). El mostrador ya
   * la sirve; solo falta encenderlo cuando los dos lados vayan juntos.
   */
  shortInvites = false
} = {}) {
  ...
  function startPairing ({ scope = defaultScope, ttlMs = defaultTtlMs, label = '', mode = 'join', account = '' } = {}) {
    ...
    const conn = shortInvites ? (typeof connToken === 'function' ? connToken() : connToken) : null
    if (conn) {
      ...
      return { token: sn, qr: { v: 2, conn, sn, m: mode }, expiresInMs: PAIRING_TTL_MS }
    }
  ...
  function handleHello (from, p) {
    ...
    reply(from, { type: MSG_HELLO_OK, iss, proxy, acct: pend.account || '', m: pend.mode || 'join' })
    return { ok: true }
  }
