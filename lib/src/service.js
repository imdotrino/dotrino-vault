...
async function openSealedBundle (sealed, ident, acta = null, masterPubkey = null) {
  ...
  for (const [key, e] of Object.entries(sealed.entries || {})) {
    ...
    if (typeof e.v === 'string' && !e.e) {
      throw Object.assign(new Error(`${key} arrived in the clear: since 2026-09-02 every variable travels sealed`), { code: 'plaintext-var' })
    }
...
export async function waitForSecrets ({ dir, ns, proxyUrl, masterPubkey, device, cert, enc, retryMs = 5000, maxRetryMs = 60000, onRetry, onPending, onCert, publicOnly = false } = {}) {
  let delay = retryMs
  for (;;) {
    try {
      return await fetchSecrets({ dir, ns, proxyUrl, masterPubkey, device, cert, enc, onPending, onCert, publicOnly })
    } catch (e) {
