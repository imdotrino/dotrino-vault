...
import { verifyActa, sealKeyAt, sealersOf } from '@dotrino/identity/acta'
 ...
 */
export async function fetchSecrets ({ dir, ns, proxyUrl, masterPubkey, device, cert, enc, timeoutMs = 30000, onPending, onCert, approvalTimeoutMs = APPROVAL_TIMEOUT_MS } = {}) {
  let saved = null
  ...
  /** Hasta qué acta hemos visto: es lo que dispara la renovación la próxima vez. */
  const anotarActa = (acta) => {
    if (typeof acta?.seq !== 'number' || saved?.actaSeq === acta.seq) return
    recordar({ actaSeq: acta.seq })
  }
...
export function isFinal (e) {
  if (e?.code === 'no-wrapping') return true
...
export async function waitForSecrets ({ dir, ns, proxyUrl, masterPubkey, device, cert, enc, retryMs = 5000, maxRetryMs = 60000, onRetry, onPending, onCert } = {}) {
  let delay = retryMs
  for (;;) {
    try {
      return await fetchSecrets({ dir, ns, proxyUrl, masterPubkey, device, cert, enc, onPending, onCert })
    } catch (e) {
