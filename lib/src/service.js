...
import {
  makeDeviceKey, makeDeviceEncKey, importDeviceEncKey, signWithDevice,
...
import { verifyActa, sealKeyAt, memberCan, checkVaultReply } from '@dotrino/identity/acta'
import { MSG, secretsScope, isValidSecretsNs, VAULT_PROTOCOL, VAULT_SPEAKS } from './protocol.js'
...
import { selfContext } from './proc.js'

const IDENTITY_FILE = 'service-identity.json'
const FRESH_WINDOW_MS = 5 * 60 * 1000

/**
 * QUÉ SOMOS, dentro de lo que ya va firmado (CONVENCIONES §14).
 *
 * Va DENTRO de `data` y no en el envoltorio a propósito: `data` lo firma la llave del
 * aparato, así que lo que la bóveda apunta y enseña es lo que ese aparato dijo **con su
 * llave**. Un inventario de versiones que cualquiera puede escribir no es un inventario.
 *
 * `PRODUCT` es el cliente de la bóveda —el que se instala con `@dotrino/env`—, que es
 * justo la pieza que se quedaba atrás sin que nadie se enterara: pedía `^0.33.2` con la
 * librería en 0.60 y enrolar contestaba `invalid cert: no-acta`, que es verdad y no es la
 * causa.
 */
const PRODUCT = 'vault'
export const MY_VERSION = {
  product: PRODUCT,
  version: PKG_VERSION,
...
export async function sayHello (client, qr) {
  ...
  const hello = await new Promise((resolve, reject) => {
    const off = client.on('message', (_f, p) => {
      ...
      else if (p?.type === MSG.ERROR) { finish(); reject(vaultError(p)) }
    })
...
/**
 * El hello de esa invitación, sin enrolar nada: devuelve el `qr` ENRIQUECIDO con
 * `iss`, `proxy` y `acct`.
 *
 * Devuelve el qr y no solo el nombre PORQUE LA CITA SE QUEMA AL CANJEARLA. Quien
 * pregunta el nombre de la cuenta y después enrola estaría canjeando dos veces la
 * misma cita, y el segundo canje falla con «código no válido o ya usado» — un error
 * que señala a la invitación cuando el problema es haberla gastado uno mismo.
 * Con el qr enriquecido, `sayHello` corta por su `if (qr.iss) return qr` y el
 * enrolamiento no vuelve a salir a preguntar.
 */
export async function vaultHello (qr) {
  ...
  try { return await sayHello(client, qr) } finally { try { client.close() } catch (_) {} }
}

/**
 * Cómo se llama la cuenta de esa invitación, sin enrolar nada.
 *
 * OJO: gasta la cita. Si además vas a enrolar, usá `vaultHello` y pasale al enroll el
 * qr que devuelve; si no, el enrolamiento se encuentra la cita quemada.
 */
export async function vaultAccountName (qr) {
  if (typeof qr === 'string') qr = parseInvite(qr)
  if (qr?.acct) return qr.acct
  return (await vaultHello(qr)).acct || ''
}
...
export async function enrollWithVault ({ qr, label = 'agent', expectedScope = null, onCode, approveTimeoutMs = 180000, withEncKey = true } = {}) {
  ...
  try {
    ...
    const enrolled = new Promise((resolve, reject) => {
      const off = client.on('message', (_from, p) => {
        ...
        if (p.type === MSG.ENROLL_CHALLENGE) {
          ...
          show({ deviceId, code })
        } else if (p.type === MSG.ENROLLED) { cleanup(); resolve(p) } else if (p.type === MSG.ERROR) { cleanup(); reject(vaultError(p)) }
      })
    ...
    //
    // Se juzga contra la llave de LA BÓVEDA del QR, con la regla del pilar. Antes se exigía
    // `acta.profileId === qr.iss`, que solo es verdad en una cuenta que nació en esa bóveda:
    // un servicio no podía emparejarse con una segunda bóveda ni con una que adoptó la cuenta.
    const chk = await checkVaultReply({
      acta: res.acta, cert: res.cert, vault: qr.iss, sub: device.publickey,
      scope: expectedScope || null, justSealed: true
    })
...
export async function registerEncKey ({ dir, ns, proxyUrl, masterPubkey, device, cert, timeoutMs = 30000 } = {}) {
  ...
  try {
    ...
    if (res.type === MSG.ERROR) throw vaultError(res)
  ...
  const anotarActa = (acta) => {
    ...
    if (renovar) {
      try {
        ...
        const res = await pending
        // SE VERIFICA ANTES DE GUARDARLO. Aquí solo se miraba que fuera para esta llave: un
        // papel cualquiera, sin firma que valga, sustituía al bueno y el servicio se quedaba
        // fuera hasta el siguiente arranque. Misma regla que al enrolarse, sin `justSealed`:
        // la última acta pudo sellarla otra selladora de la cuenta.
        const ok = res.type === MSG.RENEWED &&
          (await checkVaultReply({ acta: res.acta, cert: res.cert, vault: masterPubkey, sub: device.publickey })).ok
        if (ok) {
          cert = res.cert
          recordar({ cert })
    ...
    /**
     * QUÉ COMANDO Y DESDE QUÉ CARPETA — pero SOLO por el mostrador local.
     *
     * Quien aprueba necesita saber qué está pidiendo las claves: «el aparato 904C-1002 pide
     * el cajón proxy» no alcanza para decidir nada (dueño, 2026-09-11). Así que el pedido
     * lleva el comando y el path, y la bóveda los comprueba contra `/proc`.
     *
     * POR EL PROXIO NO VA, y no es un olvido: el pedido de secretos viaja firmado pero EN
     * CLARO (CONVENCIONES §4.1 — el transporte no cifra), así que meter ahí el `argv` y el
     * `cwd` le enseñaría al proxio las rutas de tu disco y los argumentos de tus comandos,
     * que es justo lo que este ecosistema promete que no pasa. Sin contexto no hay huella,
     * y sin huella no hay ventana: un servicio que pide desde otra máquina sigue pidiendo
     * aprobación cada vez. Se abrirá cuando el pedido vaya sellado (la deuda del §4.1).
     */
    const ctx = client.local ? selfContext() : null
    ...
    const data = { op: 'secrets', ns, ek: eph.ek, publickey: device.publickey, ts: Date.now(), v: MY_VERSION, ...(ctx ? { ctx } : {}), ...(publicOnly ? { publicOnly: true } : {}) }
    const { signature } = await signWithDevice({ privateJwk: device.privateJwk, data })
...
const FINAL_CODES = new Set([
  ...
  // NADIE EN EL ACTA PUEDE APROBAR: reintentar no lo arregla, y callarlo es lo peor. Pasó el
  // 2026-09-24: la limpieza de aparatos muertos se llevó al único teléfono con `aprueba`, y
  // `dotrino-env` se quedó reintentando cada minuto sin decir nada mientras la bóveda
  // contestaba exactamente qué había que hacer.
  'no-approver'
])

/**
 * Un `vault.error` como Error CON SU `code`. Antes se construía con el texto a secas, y el
 * código que la bóveda manda para distinguir los casos se perdía justo aquí — así que
 * `isFinal` no podía reconocer ninguno por `code` y todo se reintentaba.
 */
function vaultError (p) {
  return Object.assign(new Error(p?.error || 'vault error'), { code: p?.code || null })
}
