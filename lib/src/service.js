 ...
 * «no autorizado: revoked» y apaga al agente ahí mismo.
 *
...
export async function watchSecretsChanges ({
  ...
  const atender = async (payload) => {
    ...
    setTimeout(() => { if (!parado) { try { onChange?.({ ns, ts: body.ts, via: 'notice' }) } catch (e) { log('[vault] ' + e.message) } } }, espera)
  }
  ...
  const reconcile = async (trigger) => {
    if (parado || reconciling) return false
    ...
    const sinceStart = Date.now() - nacido
    ...
    try {
      ...
      if (/no autorizado: revoked/.test(e.message)) {
    ...
    fingerprint = current
    ultimoObedecido = Date.now()
    log(`[vault] the config for "${ns}" is not the one running (noticed on ${trigger}): the notice never arrived`)
    if (!parado) { try { onChange?.({ ns, ts: Date.now(), via: 'reconcile' }) } catch (e) { log('[vault] ' + e.message) } }
    return true
  }
