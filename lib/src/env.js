...
export function serviceDir (vault, ns) {
  ...
  if (!isValidVaultLabel(vault)) {
    throw new Error('missing vault label: pass --vault <nombre> (use [a-z0-9-]{1,32}). It is a label of YOUR machine: it says which vault this drawer came from')
  }
  if (!isValidSecretsNs(ns)) throw new Error('invalid ns (use [a-z0-9-]{1,32}, e.g. "myapp")')
  return path.join(serviceRoot(), vault, ns)
}
 ...
 */
export async function loadEnv ({ ns, vault, dir, override, wait = true, required = [], onRetry, onPending } = {}) {
  if (override === undefined) override = overrideByDefault()
  ns = resolveNs(ns)
  // La ETIQUETA DE BÓVEDA es obligatoria salvo que se diga el directorio a pelo: es lo que
  // hace que dos bóvedas puedan tener un cajón con el mismo nombre sin pisarse.
