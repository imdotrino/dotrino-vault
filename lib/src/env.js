 ...
 *
 * Es una ETIQUETA TUYA, de esta máquina, y NO tiene nada que ver con el perfil de la
 * bóveda: nadie la manda por la red, no entra en el acta y no se compara con nada. Sirve
 * solo para que dos bóvedas puedan tener un cajón que se llame igual.
 *
 ...
 */
export const isValidEnvProfile = (p) => typeof p === 'string' && /^[a-z0-9-]{1,32}$/.test(p)

/** Directorio de la identidad del servicio `ns` en el perfil local `profile`. */
export function serviceDir (profile, ns) {
  if (process.env.DOTRINO_ENV_DIR) return process.env.DOTRINO_ENV_DIR
  if (!isValidEnvProfile(profile)) {
    throw new Error('missing local profile: pass --profile <nombre> (use [a-z0-9-]{1,32}). It is a label of YOUR machine, not the vault profile')
  }
  if (!isValidSecretsNs(ns)) throw new Error('invalid ns (use [a-z0-9-]{1,32}, e.g. "myapp")')
  return path.join(serviceRoot(), profile, ns)
}

/** El perfil local por defecto: `--profile`, o `DOTRINO_ENV_PROFILE`. */
export const resolveEnvProfile = (v) => v || process.env.DOTRINO_ENV_PROFILE || null

/**
 * Lo enrolado en esta máquina: `[{ profile, ns }]`. Recorre DOS niveles.
 *
...
export function listEnrolled () {
  ...
  for (const d of dirs) {
    if (!d.isDirectory() || !isValidEnvProfile(d.name)) continue
    ...
    for (const ns of hijos) {
      if (isValidSecretsNs(ns) && readServiceIdentity(path.join(serviceRoot(), d.name, ns))) out.push({ profile: d.name, ns })
    }
  }
  return out
}
 ...
 */
export async function loadEnv ({ ns, profile, dir, override, wait = true, required = [], onRetry, onPending } = {}) {
  if (override === undefined) override = overrideByDefault()
  ns = resolveNs(ns)
  // El PERFIL LOCAL es obligatorio salvo que se diga el directorio a pelo: es lo que hace
  // que dos bóvedas puedan tener un cajón con el mismo nombre sin pisarse.
  dir = dir || serviceDir(resolveEnvProfile(profile), ns)
 ...
 */
export async function watchEnv ({ ns, profile, dir, applied = lastApplied ?? undefined, onUpdate, exitCode = 0, quiet = false, ...rest } = {}) {
  ns = resolveNs(ns)
  dir = dir || serviceDir(resolveEnvProfile(profile), ns)
