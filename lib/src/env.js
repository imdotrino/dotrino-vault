...
/**
 * DE QUÉ BÓVEDA VINO ESTO: `~/.dotrino/service/<bóveda>/<ns>`.
 *
 * Es una ETIQUETA TUYA, de esta máquina: nadie la manda por la red, no entra en el acta y
 * no se compara con nada — puedes llamarla como quieras. Sirve solo para que dos bóvedas
 * puedan tener un cajón que se llame igual.
 *
 * Se llama `--vault` y no `--profile` a propósito (dueño): `dotrino-vault --profile` ya
 * significa OTRA cosa —el perfil de la bóveda— y los dos comandos se usan seguidos. Dos
 * banderas con el mismo nombre y distinto significado se confunden el día que hay prisa.
 *
 * Antes la ruta era `<raíz>/<ns>` y el nombre del cajón era una CLAVE GLOBAL del usuario:
 * si dos bóvedas usaban `aws`, el segundo enrolamiento pisaba al primero y lo dejaba
 * inservible —avisaba, pero el papel del primero seguía existiendo en su bóveda sin
 * identidad en disco que lo usara—. Obligar a nombrar el perfil quita esa colisión de
 * raíz en vez de pedirle al usuario que se acuerde de no repetir nombres.
 *
 * Mismo alfabeto que el `ns` porque es lo mismo: un segmento de ruta.
 */
export const isValidVaultLabel = (p) => typeof p === 'string' && /^[a-z0-9-]{1,32}$/.test(p)

/** Directorio de la identidad del servicio `ns` bajo la etiqueta de bóveda `vault`. */
export function serviceDir (vault, ns) {
  ...
  if (isValidVaultLabel(vault)) return path.join(serviceRoot(), vault, ns)
  // SIN `--vault`: SE BUSCA, Y SOLO SE EXIGE SI HAY EMPATE (dueño, 2026-09-01).
  //
  // La etiqueta se hizo obligatoria para que dos bóvedas pudieran tener un cajón que se
  // llame igual sin pisarse. Eso lo arregla la RUTA, no la bandera: con un solo enrolamiento
  // de ese `ns` no hay nada que desambiguar y pedirla es fricción sin nada a cambio.
  //
  // Con dos, en cambio, elegir por el usuario sería adivinar cuál de sus cuentas usar — así
  // que se para y se dice cuáles hay. Eso es lo contrario de un repliegue: no se resuelve
  // una ambigüedad a la brava, se señala.
  const donde = listEnrolled().filter((e) => !e.legacy && e.ns === ns)
  if (donde.length === 1) return path.join(serviceRoot(), donde[0].vault, ns)
  if (donde.length > 1) {
    throw new Error(`"${ns}" is enrolled in more than one vault (${donde.map((e) => e.vault).join(', ')}): pick one with --vault <label>`)
  }
  const otros = listEnrolled().filter((e) => !e.legacy)
  throw new Error(otros.length
    ? `"${ns}" is not enrolled on this machine (there is: ${otros.map((e) => `${e.vault}/${e.ns}`).join(', ')})`
    : `"${ns}" is not enrolled on this machine: run \`dotrino-env enroll --vault <label> --ns ${ns}\` first`)
}

/** La etiqueta de bóveda: `--vault`, o `DOTRINO_ENV_VAULT`. */
export const resolveVaultLabel = (v) => v || process.env.DOTRINO_ENV_VAULT || null

/**
 * Lo enrolado en esta máquina: `[{ vault, ns }]`. Recorre DOS niveles.
 *
 * Y avisa de lo que quedó en el nivel de antes (`<raíz>/<ns>`): no se lee ni se usa —
 * repescarlo en silencio sería fingir que la migración no hace falta— pero callarlo
 * dejaría al dueño con un servicio que «desapareció» sin explicación.
 */
export function listEnrolled () {
  const out = []
  let dirs = []
  try { dirs = fs.readdirSync(serviceRoot(), { withFileTypes: true }) } catch (_) { return out }
  for (const d of dirs) {
    if (!d.isDirectory() || !isValidVaultLabel(d.name)) continue
    // Un enrolamiento del formato viejo es un directorio con `service-identity.json`
    // DENTRO; uno del nuevo tiene subdirectorios. Así se distinguen sin adivinar.
    if (readServiceIdentity(path.join(serviceRoot(), d.name))) { out.push({ legacy: true, ns: d.name }); continue }
    let hijos = []
    try { hijos = fs.readdirSync(path.join(serviceRoot(), d.name)) } catch (_) { continue }
    for (const ns of hijos) {
      if (isValidSecretsNs(ns) && readServiceIdentity(path.join(serviceRoot(), d.name, ns))) out.push({ vault: d.name, ns })
    }
  }
  return out
}
 ...
 * @param {boolean} [opts.publicOnly] Solo las variables PÚBLICAS. No hay claves privadas de
 *   por medio, así que la bóveda no pide aprobación: sirve para un arranque que solo
 *   necesita configuración (una URL, un puerto) y no debe despertar a nadie.
 ...
 */
export async function loadEnv ({ ns, vault, dir, override, wait = true, required = [], onRetry, onPending, publicOnly = false } = {}) {
  if (override === undefined) override = overrideByDefault()
  ns = resolveNs(ns)
  // La etiqueta de bóveda es OPCIONAL: `serviceDir` busca el `ns` entre las que hay y solo
  // la exige si está enrolado en más de una (ahí sí hay que elegir, y elige el dueño).
  dir = dir || serviceDir(resolveVaultLabel(vault), ns)
  const load = wait ? waitForSecrets : fetchSecrets
  const secrets = await load({ dir, ns, onRetry, onPending, publicOnly })
 ...
 */
export async function watchEnv ({ ns, vault, dir, applied = lastApplied ?? undefined, onUpdate, exitCode = 0, quiet = false, ...rest } = {}) {
  ns = resolveNs(ns)
  dir = dir || serviceDir(resolveVaultLabel(vault), ns)
