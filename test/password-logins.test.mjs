...
async function register (d, { user, password, pub = '{"kty":"EC","x":"pub"}', label = 'equipo prestado', replace = false }) {
  const start = opaqueClient.registrationStart({ password })
  const { response } = await d.registerBegin({ user, request: start.request, replace })
  ...
  await d.registerFinish({ user, upload: fin.upload, pub, label, blob, replace })
  return { exportKey: fin.exportKey, blob }
}

async function login (d, { user, password, label = '' }) {
  const start = opaqueClient.loginStart({ password })
  const { lid, response } = await d.loginBegin({ user, request: start.request })
  const fin = opaqueClient.loginFinish({ state: start.state, response, password })
  return { ...(await d.loginEnd({ lid, finalization: fin.finalization, label })), exportKey: fin.exportKey }
}

/**
 * El mostrador contesta PROMESAS desde que el OPAQUE se puede inyectar: en una extensión
 * vive en una página sandbox, al otro lado de un `postMessage`. Así que aquí se esperan
 * rechazos, no excepciones.
 */
const fails = (fn, code) => assert.rejects(async () => fn(), (e) => e.code === code, `expected ${code}`)

/** Igual que `fails`, pero devuelve el error: hace falta para mirar cuánto hay que esperar. */
async function grab (fn, code) {
...
test('the password never reaches the vault, and what it stores cannot be used to guess it', async () => {
  const { desk: d, raw } = desk()
  await register(d, { user: 'ana', password: 'la contraseña larga de ana' })
...
test('logging in returns the sealed keys, and the same password always opens them', async () => {
  const { desk: d } = desk()
  const reg = await register(d, { user: 'ana', password: 'correct horse' })
  const first = await login(d, { user: 'ana', password: 'correct horse' })
  ...
  assert.ok(first.sid)
  const second = await login(d, { user: 'ana', password: 'correct horse' })
...
test('a wrong password and a user that does not exist fail the same way', async () => {
  const { desk: d } = desk()
  await register(d, { user: 'ana', password: 'correct horse' })
  await fails(() => login(d, { user: 'ana', password: 'wrong horse' }), 'login-failed')
  await fails(() => login(d, { user: 'nadie', password: 'whatever' }), 'login-failed')
})

test('five tries, and then a wait that doubles — counted when the try STARTS', async () => {
  ...
  for (let i = 0; i < FREE_TRIES; i++) {
    ...
    await d.loginBegin({ user: 'ana', request: s.request })
  }
  const blocked = await grab(() => login(d, { user: 'ana', password: 'correct horse' }), 'too-many-tries')
  ...
  await d.loginBegin({ user: 'ana', request: s6.request })
  const again = await grab(() => login(d, { user: 'ana', password: 'correct horse' }), 'too-many-tries')
  ...
  tick(again.waitMs + 1)
  const ok = await login(d, { user: 'ana', password: 'correct horse' })
  assert.ok(ok.sid)
  assert.equal(d.list()[0].fails, 0)
})

test('the owner can clear the wait from the vault machine', async () => {
  const { desk: d } = desk()
  await register(d, { user: 'ana', password: 'p' })
  for (let i = 0; i < FREE_TRIES; i++) {
    const s = opaqueClient.loginStart({ password: 'no' })
    await d.loginBegin({ user: 'ana', request: s.request })
  }
  await fails(() => login(d, { user: 'ana', password: 'p' }), 'too-many-tries')
  assert.deepEqual(d.clearBlock({ user: 'ana' }), { ok: true })
  assert.ok((await login(d, { user: 'ana', password: 'p' })).sid, 'the wait is gone without changing the password')
  await fails(() => d.clearBlock({ user: 'beto' }), 'no-user')
})

test('the wait has a cap, so a typo does not lock you out for days', async () => {
...
test('sessions are listed and closed, and they do not expire on their own', async () => {
  const { desk: d, tick } = desk()
  await register(d, { user: 'ana', password: 'p' })
  const a = await login(d, { user: 'ana', password: 'p', label: 'cyber' })
  const b = await login(d, { user: 'ana', password: 'p' })
...
test('changing the password closes what was open and the old one stops working', async () => {
  const { desk: d } = desk()
  await register(d, { user: 'ana', password: 'vieja' })
  await login(d, { user: 'ana', password: 'vieja' })
  assert.equal(d.list()[0].sessions.length, 1)

  const nueva = await register(d, { user: 'ana', password: 'nueva', replace: true })
  assert.equal(d.list()[0].sessions.length, 0, 'the open sessions are gone')
  await fails(() => login(d, { user: 'ana', password: 'vieja' }), 'login-failed')
  const ok = await login(d, { user: 'ana', password: 'nueva' })
  assert.equal(ok.blob, nueva.blob)
})

test('the exchange in flight is single use and does not stay open for ever', async () => {
  const { desk: d, tick } = desk()
  await register(d, { user: 'ana', password: 'p' })
  const start = opaqueClient.loginStart({ password: 'p' })
  const { lid, response } = await d.loginBegin({ user: 'ana', request: start.request })
  const fin = opaqueClient.loginFinish({ state: start.state, response, password: 'p' })
  await d.loginEnd({ lid, finalization: fin.finalization })
  await fails(() => d.loginEnd({ lid, finalization: fin.finalization }), 'no-exchange')

  const s2 = opaqueClient.loginStart({ password: 'p' })
  const x2 = await d.loginBegin({ user: 'ana', request: s2.request })
  const f2 = opaqueClient.loginFinish({ state: s2.state, response: x2.response, password: 'p' })
  tick(3 * 60_000)
  await fails(() => d.loginEnd({ lid: x2.lid, finalization: f2.finalization }), 'no-exchange')
})

test('names are checked, and nothing is created twice by accident', async () => {
  const { desk: d } = desk()
  await register(d, { user: 'ana', password: 'p' })
  await fails(() => register(d, { user: 'ana', password: 'otra' }), 'user-exists')
  await fails(() => register(d, { user: 'Ana Pérez', password: 'p' }), 'bad-user')
  await fails(() => register(d, { user: 'beto', password: 'p', replace: true }), 'no-user')
  await fails(() => d.setCert({ user: 'beto', cert: {} }), 'no-user')
})

test('who is logged in: a key with an open session is told apart from one without', async () => {
  const { desk: d } = desk()
  await register(d, { user: 'ana', password: 'p', pub: 'KEY-A' })
  assert.equal(d.hasOpenSession({ pub: 'KEY-A' }), false)
  const s = await login(d, { user: 'ana', password: 'p' })
...
test('removing the login takes its record and its sessions with it', async () => {
  const { desk: d, raw } = desk()
  await register(d, { user: 'ana', password: 'p' })
  await login(d, { user: 'ana', password: 'p' })
  assert.deepEqual(d.remove({ user: 'ana' }), { ok: true })
  assert.deepEqual(d.list(), [])
  assert.equal(JSON.stringify(raw().users), '{}')
  await fails(() => login(d, { user: 'ana', password: 'p' }), 'login-failed')
})
