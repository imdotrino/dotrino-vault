...
import { agruparPorAparato } from '../src/vaultControl.js'

const cert = (sub, nonce, dias, extra = {}) => ({ sub, nonce, exp: Date.now() + dias * 864e5, label: 'pc-local', ...extra })

test('las renovaciones del mismo aparato son UNA fila, con todos sus nonces', () => {
  const r = agruparPorAparato([cert('K1', 'viejo', 5), cert('K1', 'nuevo', 30)], [])
...
test('un certificado revocado ya no cuenta como dispositivo enrolado', () => {
  ...
  const r = agruparPorAparato([cert('K1', 'viejo', 5), cert('K1', 'nuevo', 30)], [{ nonce: 'viejo' }])
  ...
  assert.equal(agruparPorAparato([cert('K1', 'a', 5)], [{ nonce: 'a' }]).length, 0)
  assert.equal(agruparPorAparato([cert('K1', 'a', 5, { revokedAt: Date.now() })], []).length, 0, 'también por la marca del propio registro')
})

test('aparatos distintos siguen siendo filas distintas', () => {
  const r = agruparPorAparato([cert('K1', 'a', 30), cert('K2', 'b', 30)], [])
  assert.equal(r.length, 2)
})

test('sin `sub` no se fusiona nada (un cert suelto es su propia fila)', () => {
  const r = agruparPorAparato([cert(null, 'a', 30), cert(null, 'b', 30)], [])
...
test('revoked puede venir como lista de cadenas o de objetos', () => {
  assert.equal(agruparPorAparato([cert('K1', 'a', 5)], ['a']).length, 0)
  assert.equal(agruparPorAparato([cert('K1', 'a', 5)], [{ nonce: 'a' }]).length, 0)
})
