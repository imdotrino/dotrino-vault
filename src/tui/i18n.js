...
const es = {
  ...
  tabLogins: 'Entrar',
  noLogins: 'Todavía no hay ninguno. Con «n» creas uno.',
  loginsHint: 'Un aparato de esta cuenta que se abre escribiendo una dirección y una contraseña, sin conectar nada. Sirve para entrar desde un equipo prestado.',
  loginOpen: (n) => `${n} abierto${n === 1 ? '' : 's'}`,
  loginWait: (s) => `espera ${s} s`,
  loginCreated: (d) => `creado ${d}`,
  helpLogins: ({ hasLogins, hasSessions, blocked } = {}) => [
    '←→ pestaña', '↑↓', 'n nuevo',
    ...(hasSessions ? ['x cerrar sesiones'] : []),
    ...(blocked ? ['u quitar espera'] : []),
    ...(hasLogins ? ['c contraseña', 'v quitar'] : []),
    'F5 refrescar', 'Esc bóvedas', 'l English', 'q salir'
  ],
  loginUserLabel: '¿Qué nombre de usuario?',
  loginUserHint: 'letras y números, en minúscula (Esc cancela)',
  loginLabelLabel: (u) => `¿De dónde va a entrar «${u}»?`,
  loginLabelHint: 'lo que vas a leer aquí para decidir si cerrarlo (Esc cancela)',
  loginPassLabel: (u) => `Contraseña para «${u}»`,
  loginPassAgain: 'Escríbela otra vez',
  loginPassHint: 'mínimo 12 caracteres; no se puede recuperar (Esc cancela)',
  loginPassShort: 'La contraseña tiene que tener al menos 12 caracteres.',
  loginPassMismatch: 'No coinciden. Inténtalo otra vez.',
  loginOldPass: (u) => `Contraseña actual de «${u}»`,
  loginNewPass: 'Contraseña nueva',
  loginCreating: 'Creando el inicio de sesión…',
  loginCreatedOk: (a) => `Listo. La dirección es ${a}`,
  loginChanging: 'Cambiando la contraseña…',
  loginChanged: 'Contraseña cambiada.',
  loginClosing: 'Cerrando las sesiones…',
  loginClosed: (n) => `${n} sesión(es) cerrada(s).`,
  loginUnblocking: 'Quitando la espera…',
  loginUnblocked: 'Ya puede volver a intentarlo.',
  loginRemoving: 'Quitándolo…',
  loginRemoved: (u) => `«${u}» se fue, y su llave salió del acta.`,
  loginRemoveConfirm: (u) => `¿Quitar «${u}»? Su llave sale del acta y ese equipo deja de entrar.`,
  loginNoSessions: 'Ese no tiene ninguna sesión abierta.',
  loginNotBlocked: 'Ese no está esperando.',
  ...
  privateData: 'Privados',
  privateDataHint: '  (sellados: se ven en tus aparatos, no aquí)',
  hidden: '   (oculto)',
  no: 'no',
...
const en = {
  ...
  tabLogins: 'Sign in',
  noLogins: 'None yet. Press “n” to create one.',
  loginsHint: 'A device of this account that opens by typing an address and a password, with nothing to plug in. It is how you sign in from a borrowed computer.',
  loginOpen: (n) => `${n} open`,
  loginWait: (s) => `wait ${s} s`,
  loginCreated: (d) => `created ${d}`,
  helpLogins: ({ hasLogins, hasSessions, blocked } = {}) => [
    '←→ tab', '↑↓', 'n new',
    ...(hasSessions ? ['x close sessions'] : []),
    ...(blocked ? ['u clear wait'] : []),
    ...(hasLogins ? ['c password', 'v remove'] : []),
    'F5 refresh', 'Esc vaults', 'l español', 'q quit'
  ],
  loginUserLabel: 'Which username?',
  loginUserHint: 'lowercase letters and numbers (Esc cancels)',
  loginLabelLabel: (u) => `Where will “${u}” sign in from?`,
  loginLabelHint: 'what you will read here to decide whether to close it (Esc cancels)',
  loginPassLabel: (u) => `Password for “${u}”`,
  loginPassAgain: 'Type it again',
  loginPassHint: 'at least 12 characters; it cannot be recovered (Esc cancels)',
  loginPassShort: 'The password must be at least 12 characters.',
  loginPassMismatch: 'They do not match. Try again.',
  loginOldPass: (u) => `Current password for “${u}”`,
  loginNewPass: 'New password',
  loginCreating: 'Creating the sign-in…',
  loginCreatedOk: (a) => `Done. The address is ${a}`,
  loginChanging: 'Changing the password…',
  loginChanged: 'Password changed.',
  loginClosing: 'Closing the sessions…',
  loginClosed: (n) => `${n} session(s) closed.`,
  loginUnblocking: 'Clearing the wait…',
  loginUnblocked: 'It can try again now.',
  loginRemoving: 'Removing it…',
  loginRemoved: (u) => `“${u}” is gone, and its key left the account record.`,
  loginRemoveConfirm: (u) => `Remove “${u}”? Its key leaves the account record and that computer stops getting in.`,
  loginNoSessions: 'That one has no open sessions.',
  loginNotBlocked: 'That one is not waiting.',
  tabDevices: 'Devices',
  ...
  privateData: 'Private',
  privateDataHint: '  (sealed: they show on your devices, not here)',
  hidden: '   (hidden)',
  noName: '(no name)',
  no: 'no',
