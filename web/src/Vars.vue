...
watch(() => props.rows, (rows) => {
  draft.value = (rows || []).map((r) => {
    ...
    // EL VALOR DE UNA PÚBLICA SÍ SE ENSEÑA (dueño, 2026-09-02): la bóveda nos manda su sobre
    // y `loadVars` lo abre. Una privada no trae valor y su caja queda vacía con `••••••`.
    const value = r.value ?? ''
